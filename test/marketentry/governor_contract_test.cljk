  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Roseau Trading Co. Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Roseau Trading Co. Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest award-authority-insufficient-is-held-and-unoverridable
  (testing "approving-authority falls short of what the contract value requires -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:award-authority-insufficient} (-> (store/ledger db) last :basis)))
...
(deftest paye-unregistered-is-held-and-unoverridable
  (testing "unregistered PAYE when the engagement has employees -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:paye-unregistered} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
