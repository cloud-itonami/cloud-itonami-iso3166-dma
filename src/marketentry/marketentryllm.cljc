(ns marketentry.marketentryllm
  ...
  the Commonwealth of Dominica (DMA) public-sector market-entry
  compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "confirming-authority=" (:confirming-authority e)
                        " approving-authority=" (:approving-authority e)
                        " contract-value=" (:contract-value e)
                        " has-employees?=" (:has-employees? e)
                        " paye-registered?=" (:paye-registered? e)
