...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (DMA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up award-authority-insufficient) ==")
    ...
    (println "== filing/submit eng-4 (approving-authority-insufficient, EC$1.5M needs Public Procurement Board -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up paye-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (paye-unregistered -> HARD hold) ==")
